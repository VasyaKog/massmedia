...
use Symfony\Component\HttpFoundation\Session\Storage\Handler\PdoSessionHandler as Base;
...
class PdoSessionHandler implements \SessionHandlerInterface
{
    ...
    /**
     * @var string Column for ip
     */
    private $ipCol = 'sess_ip';

    /**
     * @var string Column for ip
     */
    private $browserCol = 'sess_browser';
    ...
    public function createTable()
    {
        ...
        switch ($this->driver) {
            case 'mysql':
                ...
                $sql = "CREATE TABLE $this->table ($this->idCol VARBINARY(128) NOT NULL PRIMARY KEY, $this->dataCol BLOB NOT NULL, $this->lifetimeCol MEDIUMINT NOT NULL, $this->timeCol INTEGER UNSIGNED NOT NULL, $this->ipCol VARCHAR(16) NOT NULL, $this->browserCol VARCHAR(32) NOT NULL) COLLATE utf8_bin, ENGINE = InnoDB";
                break;
            case 'sqlite':
            ...
            if (!$updateStmt->rowCount()) {
                try {
                    $insertStmt = $this->pdo->prepare(
                        "INSERT INTO $this->table ($this->idCol, $this->dataCol, $this->lifetimeCol, $this->timeCol, $this->ipCol, $this->browserCol) VALUES (:id, :data, :lifetime, :time, :ip, :browser)"
                    );
                    $insertStmt->bindParam(':id', $sessionId, \PDO::PARAM_STR);
                    $insertStmt->bindValue(':data', '', \PDO::PARAM_LOB);
                    $insertStmt->bindValue(':lifetime', 0, \PDO::PARAM_INT);
                    $insertStmt->bindValue(':time', time(), \PDO::PARAM_INT);
                    $insertStmt->bindValue(':ip', $_SERVER["REMOTE_ADDR"], \PDO::PARAM_INT);
                    $insertStmt->bindValue(':browser', explode(' ', $_SERVER["HTTP_USER_AGENT"])[0], \PDO::PARAM_INT);
                    $insertStmt->execute();
                } catch (\PDOException $e) {
